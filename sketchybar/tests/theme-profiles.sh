...
# Resolver defaults to the Night City blue profile without a marker.
assert_eq '0xe6070b1a' "$(resolve_value ISLAND_BG)" "missing marker defaults to Gentleman Blue island background"
assert_eq '0xff347aff' "$(resolve_value PRIMARY)" "Gentleman Blue uses electric blue as its primary color"
assert_eq '0x99347aff' "$(resolve_value GLOW_BLUE)" "Gentleman Blue exposes the electric-blue glow"
assert_eq '0x33347aff' "$(resolve_value SELECTED_BG)" "Gentleman Blue exposes the translucent selected background"
assert_eq '0xffdbe9ff' "$(resolve_value TEXT)" "Gentleman Blue uses the Night City foreground"
assert_eq '0xff4a5578' "$(resolve_value DIM)" "Gentleman Blue uses the Night City dim color"
assert_eq '0xff1c2c54' "$(resolve_value ISLAND_BORDER)" "Gentleman Blue uses the Night City border"

# Explicit Gentleman selection preserves its existing palette.
printf 'gentleman\n' > "$MARKER_DIR/sketchybar-theme"
assert_eq '0xff121620' "$(resolve_value ISLAND_BG)" "Gentleman island background remains unchanged"
...
for profile in gentleman gentleman-cute gentleman-blue; do
...
printf 'not-a-theme\n' > "$MARKER_DIR/sketchybar-theme"
assert_eq '0xe6070b1a' "$(resolve_value ISLAND_BG)" "invalid marker defaults to Gentleman Blue"
...
# The selector accepts every supported profile and publishes the marker before reload.
printf 'gentleman\n' > "$MARKER_DIR/sketchybar-theme"
: > "$LOG_FILE"
...
assert_eq 'gentleman-blue' "$(cat "$MARKER_DIR/sketchybar-theme")" "selector publishes Gentleman Blue"
assert_eq '--reload' "$(cat "$LOG_FILE")" "selector reloads after publishing Gentleman Blue"
assert_no_candidates
...
for plugin in ai_quota.sh ai_toggle.sh battery.sh cpu.sh current_apps.sh github.sh gpu.sh media.sh meeting.sh mic.sh mic_toggle.sh music.sh ram.sh space.sh volume.sh wifi.sh; do
...
# A controlled default run proves the Night City layout renders the blue profile.
rm -f "$MARKER_DIR/sketchybar-theme"
blue_main_output=$(run_config)
assert_contains "color=0x00000000" "$blue_main_output" "Night City bar keeps its transparent base"
assert_contains "icon.color=0xff5ce1ff" "$blue_main_output" "default icon uses the blue profile cyan"
assert_contains "label.color=0xff4a5578" "$blue_main_output" "workspace labels use the blue profile dim color"
assert_contains "icon.color=0xff347aff" "$blue_main_output" "separator uses the blue profile primary color"
assert_contains "label.color=0xff05070f" "$blue_main_output" "front app uses the blue profile background text"
assert_contains "background.color=0xff347aff" "$blue_main_output" "front app uses the blue profile primary background"
assert_eq '0xff347aff' "$(icon_color_for_item cpu "$blue_main_output")" "Blue initial CPU renders its profile token"

# Explicit Gentleman selection continues to render its preserved colors.
printf 'gentleman\n' > "$MARKER_DIR/sketchybar-theme"
